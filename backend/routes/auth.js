...
router.post("/register", async (req, res) => {
  try {
    ...
    // Only Student and Faculty can register publicly
    if (!["STUDENT", "FACULTY"].includes(userRole)) {
      return res.status(403).json({
        message:
          "Administrator and Approver accounts cannot be created through public registration",
      });
    }
...
router.post("/create-admin", async (req, res) => {
  try {
    ...
    // Create admin with NORMAL password
    const result = await pool.query(
      `INSERT INTO users
      (
        name,
